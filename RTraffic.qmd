...
for (i in year){
  ...
  aptData <- rbind(df,read.csv(x[[2]], sep = ";", dec = ",")) #empile le dataframe sur le précédent
  }
aptData <- aptData %>% 
...
On géénralise le cas précédent avec une fonction prenant en entrée le type de données que l'on veut extraire (par aéroports, par liaisons ou par compagnies) et on concatène sur toutes les années à disposition de 2018 à 2022.
